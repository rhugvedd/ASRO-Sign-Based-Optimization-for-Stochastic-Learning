...
class Trainer:
    ...
    def train(self):
        ...
        if (self.config.optimizer_name == ACC_ASRO_FINAL_SCALE) or (self.config.optimizer_name == ASRO) or (self.config.optimizer_name == PLAIN_CUSTOM_ADAM):
            lr_schedule = (torch.ones(self.config.num_iters + 1) * self.config.start_lr)
            lr_schedule[:self.config.warmup_iters] = self.config.start_lr * torch.arange(self.config.warmup_iters) / self.config.warmup_iters
        elif (self.config.optimizer_name == CUSTOM_ADAM) or (self.config.optimizer_name == MOMENTUM) or (self.config.optimizer_name == R_ADAM) or (self.config.optimizer_name == ADA_GRAD) or (self.config.optimizer_name == AMS_GRAD) or (self.config.optimizer_name == RMS_PROP) or (self.config.optimizer_name == SGD_NESTEROV):
